...
public class ConsoleTableRenderer implements Renderer {
    ...
    public void render() {
        ...
        for (Product p : products) {
            String[] row = {
                    ...
                    p.getImageUrl(),
                    String.format("%tF", p.getCreatedAt()),
                    String.format("%tF", p.getUpdatedAt()),
            };
            printRow(row, widths);
        }
    ...
    private int[] calcWidths(ArrayList<Product> products) {
        ...
        for (Product p : products) {
            widths[0] = Math.max(widths[0], String.valueOf(p.getProductId()).length());
            widths[1] = Math.max(widths[1], p.getName().length());
            widths[2] = Math.max(widths[2], p.getDescription().length());
            widths[3] = Math.max(widths[3], String.valueOf(p.getCategoryId()).length());
            widths[4] = Math.max(widths[4], String.format("%.2f", p.getPrice()).length());
            widths[5] = Math.max(widths[5], String.valueOf(p.getStockQuantity()).length());
            widths[6] = Math.max(widths[6], p.getImageUrl().length());
        }
